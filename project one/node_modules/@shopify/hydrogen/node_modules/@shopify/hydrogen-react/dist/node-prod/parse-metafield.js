...
function parseMetafield(metafield) {
	if (!metafield.type) {
		console.error(`parseMetafield(): The 'type' field is required in order to parse the Metafield. Returning 'parsedValue' of 'null'`);
		return {
			...metafield,
			parsedValue: null
		};
	...
	switch (metafield.type) {
		...
		case "money": {
			...
			try {
				...
				console.error(`parseMetafield(): attempted to JSON.parse the 'metafield.value' property, but failed. Returning 'null' for 'parsedValue'`);
				parsedValue = null;
			}
			return {
				...metafield,
		...
		case "list.date":
		case "list.date_time": {
			const jsonParseValue = parseJSON(metafield?.value ?? "");
			return {
				...metafield,
		...
		case "list.variant_reference": return {
			...metafield,
			parsedValue: require_flatten_connection.flattenConnection(metafield.references ?? void 0)
		};
		default: {
			...
			console.error(`${typeNotFoundError}  Returning 'parsedValue' of 'null'`);
			return {
				...metafield,
				parsedValue: null
			};
